...
"""
This file contain code implementation for the lab in cours of Ecole Polytechnique


Few notes about data:
- unique_tokens.txt : list of unique words
- word_vectors.csv : vectors which corresponding to word in unique_token.txt
...
"""
1. Load the word vectors and compute the cosine similarity in the embedding 
space between semantically close words (e.g., “man” and “woman”) and between unrelated words. What do you observe?
"""
...
"""
2. Project the word vectors into a lower-dimensional space using PCA and visualize some regularities on the first two principal directions. 
You can use sklearn’s PCA function and its fit.transform method, and the custom plot_points function. What do you observe?
"""
...
np.round_(cosine(operation, Germany),5)
...
np.round_(cosine(operation, cars),5)
...
"""
3. Compute the cosine similarity of the two sentences “Kennedy was shot in Texas” and “The President was killed in Dallas” in the traditional vector space (“bag-of-words”). 
What do you observe? Using this time the cosine similarity of the centroids of the sentences in 
the embedding space, what do you observe? Note that the two sentences have no word in common.
...
similarity = np.round_(cosine(np.array(p_1).reshape(1,-1),np.array(p_2).reshape(1,-1)), 5)
print("The similarity between 2 sentences before doing word embedding is {}".format(similarity))
...
similarity_word_embedding = np.round_(cosine(centroid_1, centroid_2),5)
